...
for i in range(1,nusers):
    ...
    # for b in basedirs:
    #     cmds = list()
    #     cmds.append(f'sudo mkdir -p {b}/{username}')
    #     cmds.append(f'sudo chgrp tljh {b}/{username}')
    #     cmds.append(f'sudo chown {username} {b}/{username}')
    #     for c in cmds:
    #         if runcommand:
    #             subprocess.run([c], shell=True, capture_output=True, text=True)
    #         else:
