    ...
    # https://xkcd.com/418/
        ...
        flopslist = {
            ...
            '1000 years on a laptop': 5e12/128*0.5*3.15e10,
             ...
             '1 second on Setonix supercomputer':42e15,
             '1 second on Frontier supercomputer or \n 2 years on a laptop':1e18,
             '1 second on all Google cloud computing': 3.98 * 1e21,
...
#             '1 year using Earth\'s computers':3e30,
        ...
        ax.plot(x, memcalc(x, num_gates), linewidth = 4, zorder = 2, color='cornflowerblue', marker='None')
        ax.scatter([num_qubits], [memcalc(num_qubits, num_gates)], 
                   zorder = 3, facecolor='Navy', edgecolor='LightBlue', marker='o', s=100)
        ax.plot([0,num_qubits], [mem, mem], 
                linewidth = 2, zorder = 1, color='lightblue', marker='None', linestyle='dashed')
        ax.plot([num_qubits,num_qubits], [-2, mem], 
                linewidth = 2, zorder = 1, color='lightblue', marker='None', linestyle='dashed')
        ax.set_xscale('log')
        ax.set_xlim([0,2**(maxlognqubit+0.1)])
        ax.set_ylim([-2, np.max([mem*1.1, (memlist['All the storage on Earth'])])])

        ax.set_yticks([memlist[k] for k in memlist.keys()], 
                      labels=memlist.keys()) 
        ax.set_xticks([1, 2, 4, 8, 16, 32, 64, 128, 256],
                      labels=['1', '2', '4', '8', '16', '32', '64', '128', '256']
                     ) 
        ax.set_xlabel('Number of Qubits')
        ax.set_ylabel('Amount of Memory')
        ax.set_title('How much does it take to simulate on a classical computer?')
        fig.show()


        fig = plt.figure()
        ax = fig.add_axes((0.1, 0.1, 0.9, 0.9))
        ax.plot(x, flopcalc(x, num_gates, num_measurements), linewidth = 4, zorder = 2, color='darkorange', marker='None')
        ax.scatter([num_qubits], [flopcalc(num_qubits, num_gates, num_measurements)], 
                   zorder = 3, facecolor='darkgoldenrod', edgecolor='gold', marker='o', s=100)
        
        ax.plot([0,num_qubits], [flops, flops], 
                linewidth = 2, zorder = 1, color='gold', marker='None', linestyle='dashed')
        ax.plot([num_qubits, num_qubits], [-10, flops], 
                linewidth = 2, zorder = 1, color='gold', marker='None', linestyle='dashed')
        ax.set_xscale('log')
        ax.set_yticks([np.log(flopslist[k]) for k in flopslist.keys()], 
                      labels=flopslist.keys())
        ax.set_xticks([1, 2, 4, 8, 16, 32, 64, 128, 256],
                      labels=['1', '2', '4', '8', '16', '32', '64', '128', '256']
                     ) 
        ax.set_xlim([0,2**(maxlognqubit+0.1)])
        ax.set_ylim([-10, np.max([flops*1.1, np.log(flopslist[list(flopslist.keys())[-1]])])])

        ax.set_xlabel('Number of Qubits')
        ax.set_ylabel('Amount of Time')
        ax.set_title('How long does it take to simulate on a classical computer?')
        fig.show()
